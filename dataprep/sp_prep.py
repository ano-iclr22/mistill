...
def _get_neighbors(graph: nx.Graph, u: Any, neighbor_to_idx: Dict[Any, Dict[Any, int]]) -> np.array:
    ...
    """
    max_deg = np.max(list(dict(graph.degree()).values())) / 2
    # neighbors = np.repeat(-1, repeats=len(neighbor_to_idx[u])).astype(np.int32)
    neighbors = np.repeat(-1, repeats=max_deg).astype(np.int32)
    for v, idx in neighbor_to_idx[u].items():
        neighbors[idx] = graph.nodes[v]['idx']
    return neighbors
...
def _get_mask(u: Any, neighbor_to_idx: Dict[Any, Dict[Any, int]], num_outputs: int) -> np.array:
    ...
    """
    mask = np.zeros(num_outputs, dtype=np.float32)
    for idx in neighbor_to_idx[u].values():
        mask[idx] = 1
    return mask


def _get_sample(graph, u, dst, num_outputs, adj, value_index):
    sample = {
        H5_TARGET: _get_distributional_output(graph, u, dst, num_outputs, adj, value_index),
...
def _get_sample_mp(args: Dict[str, Any]) -> Dict[str, np.array]:
    ...
    for u, dst in pairs:
        sample = _get_sample(graph, u, dst, num_outputs, adj, value_index)
...
def distributional_spf_dataset(graph: nx.Graph, nodes=None, use_mp=False) -> Dict[str, np.array]:
    """
    ...
    if use_mp:
        ...
        for dst in nodes:
            for u in graph.nodes():
                if u == dst:
                    continue
                samples.append(_get_sample(graph, u, dst, num_outputs, adj, value_index))
...
def add_index_to_nodes(graph: nx.DiGraph) -> nx.DiGraph:
    """
